...
# GitHub imports with error handling
try:
    import github
    ...
    print("Install PyGithub with: pip install PyGithub==1.59.1")
...
from config.github_config import GitHubConfig, github_config
...
class GitHubService:
    ...
    def __init__(self, config: Optional[GitHubConfig] = None):
        self.config = config or github_config
        self.temp_dirs_to_cleanup = []
        
        # Check if dependencies are available
        if not GITHUB_AVAILABLE or not GIT_AVAILABLE:
            logger.warning("GitHub or Git dependencies not available. PR creation will be disabled.")
    ...
    def _initialize_github_client(self) -> Optional[Github]:
        ...
        if not GITHUB_AVAILABLE:
            logger.error("PyGithub not available - install with: pip install PyGithub==1.59.1")
            return None
        ...
        # TODO: Add GitHub App authentication support
        if self.config.github_app_id and self.config.github_app_private_key_path:
            logger.info("GitHub App authentication not yet implemented")
            return None
    ...
    def is_available(self) -> bool:
        """Check if GitHub service is available and authenticated."""
        return GITHUB_AVAILABLE and GIT_AVAILABLE and self.github_client is not None
    ...
    def create_security_pr(
        ...
        **pr_options
        ...
        Args:
            repo_url: GitHub repository URL
            dependency_updates: List of dependency updates to apply
            vulnerability_results: Raw vulnerability scan results
            **pr_options: Additional PR options (reviewers, labels, etc.)
    ...
    def _create_pull_request(
        ...
        **pr_options
        ...
        # Set PR options
        base_branch = pr_options.get('target_branch', self.config.default_target_branch)
        draft = pr_options.get('draft', self.config.enable_draft_prs)
        ...
        # Add labels
        labels = pr_options.get('labels', self.config.default_pr_labels)
        if labels:
            pr.add_to_labels(*labels)
        
        # Add reviewers
        reviewers = pr_options.get('reviewers', self.config.default_pr_reviewers)
        if reviewers:
            # Filter out team mentions (start with @) for individual reviewers
            individual_reviewers = [r for r in reviewers if not r.startswith('@')]
            if individual_reviewers:
                try:
                    pr.create_review_request(reviewers=individual_reviewers)
                except Exception as e:
